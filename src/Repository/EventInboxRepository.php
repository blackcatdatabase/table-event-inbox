...
namespace BlackCat\Database\Packages\EventInbox\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EventInbox\Definitions;
use BlackCat\Database\Packages\EventInbox\Criteria;
use BlackCat\Database\Packages\EventInbox\Dto\EventInboxDto as Dto;
use BlackCat\Database\Packages\EventInbox\Mapper\EventInboxDtoMapper as RowMapper;
...
final class EventInboxRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EventInbox\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'payload', 'status', 'attempts', 'last_error', 'processed_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'source', 'event_key' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EventInbox\Definitions::table());
            return $bulk->upsertMany($rows, [ 'source', 'event_key' ], [ 'payload', 'status', 'attempts', 'last_error', 'processed_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EventInbox\Dto\EventInboxDto|null */
    public function getBySourceAndEventKey(string $source, string $eventKey, bool $asDto = false): array|\BlackCat\Database\Packages\EventInbox\Dto\EventInboxDto|null {
        $row = $this->getByUnique([ 'source' => $source, 'event_key' => $eventKey ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EventInbox\Mapper\EventInboxDtoMapper::fromRow($row);
    }
    public function existsBySourceAndEventKey(string $source, string $eventKey): bool {
        $where = 't.' . Ident::q($this->db, 'source') . ' = :uniq_source' . ' AND ' . 't.' . Ident::q($this->db, 'event_key') . ' = :uniq_event_key';
        return $this->exists($where, [ 'uniq_source' => $source, 'uniq_event_key' => $eventKey ]);
    }
    /** @return int|string|null */
    public function getIdBySourceAndEventKey(string $source, string $eventKey) {
        $row = $this->getBySourceAndEventKey($source, $eventKey);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EventInbox\Dto\EventInboxDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EventInbox\Dto\EventInboxDto|null {
