...
use BlackCat\Database\Packages\EventInbox\Repository\EventInboxRepositoryInterface;
...
class EventInboxRepository implements EventInboxRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'source', 'event_key' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\EventInbox\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'payload', 'status', 'attempts', 'last_error', 'processed_at' ]);
          return count($rows);
        }
    ...
    public function getBySourceAndEventKey(string $source, string $eventKey, bool $asDto = false): array|\BlackCat\Database\Packages\EventInbox\Dto\EventInboxDto|null {
        return $this->getByUnique([ 'source' => $source, 'event_key' => $eventKey ], $asDto);
    }
    ...
    public function getIdBySourceAndEventKey(string $source, string $eventKey) {
        $row = $this->getBySourceAndEventKey($source, $eventKey, false);
