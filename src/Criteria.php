...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'source', 'event_key', 'payload', 'status', 'attempts', 'last_error', 'received_at', 'processed_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'source', 'event_key', 'status', 'last_error' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'source', 'event_key', 'status', 'attempts', 'last_error', 'received_at', 'processed_at' ];
}
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\EventInbox\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\EventInbox\Definitions::softDeleteColumn();
