...
namespace BlackCat\Database\Packages\EventInbox\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\EventInbox\Dto\EventInboxDto;
use BlackCat\Database\Packages\EventInbox\Mapper\EventInboxDtoMapper;
 ...
 */
final class EventInboxAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EventInboxRepository $eventInboxRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'event_inbox' => $this->eventInboxRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\EventInbox\ModuleException("Record {$idStr} not found for locking.");
            }
